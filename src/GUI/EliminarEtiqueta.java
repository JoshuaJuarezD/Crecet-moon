...
public class EliminarEtiqueta extends javax.swing.JFrame {
    ...
    IO io = new IO();
    ArrayList<Tag> Tags = io.lecturaTags();
    ...
    private void initComponents() {
        ...
        jLabel6.setText("Eliminar Etiqueta");
        jLabel6.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        getContentPane().add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 330, 170, 30));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (seleccionados.length == 0) {
            JOptionPane.showMessageDialog(new JFrame(), "Ninguna etiqueta seleccianda", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        for (int c = 0; c < seleccionados.length; c++) {
            String idtag = jTable1.getValueAt(seleccionados[c], 0).toString();
            for (int d = 0; d < Tags.size(); d++) {
                if (Tags.get(d).getID().equals(idtag) && Tags.get(d).isActivo()) {
                    Tags.get(d).setActivo(false);
                }
            }
        }
    ...
    private void refrehTable() {
        ...
        for (int c = 0; c < Tags.size(); c++) {
            if (Tags.get(c).isActivo()) {
                model.addRow(new Object[]{Tags.get(c).getID(), Tags.get(c).getNombre()});
            }
        }
