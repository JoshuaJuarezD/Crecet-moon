...
public class EliminarProducto extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 270, 170, 30));
        ...
        jButton8.setText("Eliminar etiqueta");
        jButton8.setBorder(null);
        jButton8.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        getContentPane().add(jButton8, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 330, 170, 30));
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 130, 510, 270));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (seleccionados.length == 0) {
            JOptionPane.showMessageDialog(new JFrame(), "Ningun producto seleccianda", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        for (int c = 0; c < seleccionados.length; c++) {
            String idtag = jTable1.getValueAt(seleccionados[c], 0).toString();
            for (int d = 0; d < Producto.size(); d++) {
                if (Producto.get(d).getID().equals(idtag) && Producto.get(d).isActivo()) {
                    Producto.get(d).setActivo(false);
                }
            }
        }
        io.escrituraProducto(Producto);
        refrehTable();
        JOptionPane.showMessageDialog(new JFrame(), "Producto eliminado carrectamente", "Etiqueta eliminada", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        for (int c = 0; c < model.getRowCount(); c++) {
            if (model.getValueAt(c, 0).toString().toLowerCase().contains(Busqueda) || model.getValueAt(c, 1).toString().toLowerCase().contains(Busqueda) || model2.getValueAt(c, 2).toString().toLowerCase().contains(Busqueda)) {
    ...
    private void refrehTable() {
        ...
        for (int c = 0; c < Producto.size(); c++) {
            if (Producto.get(c).isActivo()) {
                model.addRow(new Object[]{Producto.get(c).getID(), Producto.get(c).getNombre(),fomatotag(Producto.get(c).getIDtag())});
            }
        }
