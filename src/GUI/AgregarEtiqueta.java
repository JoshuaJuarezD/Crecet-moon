        ...
        if (nombre.isEmpty()) {
                ...
                for (int c = 0; c < TagExistentes.size(); c++) {
                    if (etiquetas[d].toLowerCase().equals(TagExistentes.get(c).getNombre().toLowerCase())) {
                        if(TagExistentes.get(c).isActivo())
                            JOptionPane.showMessageDialog(new JFrame(), "La etiqueta \""+ etiquetas[d]+"\" ya existe en el sistema", "Error", JOptionPane.ERROR_MESSAGE);
                        else{
            ...
            if (A == JOptionPane.YES_OPTION) {
                for(int c=0;c<etiquetas.length;c++){
                    Tag nuevoTag = new Tag("TG0" + TagExistentes.size() , etiquetas[c], true);
